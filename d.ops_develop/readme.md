...
bash d.ops_develop/h.op_agent/a.install_opencode/run.sh      # 安装 opencode 绿色版
bash d.ops_develop/h.op_agent/b.skill_store/run.sh           # 离线选装 Skill
...
- `h.op_agent` 是 OpenCode 的运行时 + 昇腾官方 skill 的离线仓库/在线更新；CANN toolkit、torch 等环境依赖由「容器 + 环境检查」提供建议，按需到 `f.installation` 补装。
